...
public class ObjReader {
    ...
    public static List<Obj3D> readMultiObj(Context context, String file){
        ...
        ArrayList<Float> v=new ArrayList<Float>();//原始顶点坐标列表
        ArrayList<Float> vn=new ArrayList<>();    //原始顶点法线列表
        ArrayList<Float> vt=new ArrayList<>();    //原始贴图坐标列表
            ...
            while((temps=br.readLine())!=null){
                if("".equals(temps)){
                    ...
                    switch (tempsa[0].trim()){
                        ...
                        case "f":
                            fCount++;
                            //面 f 22826/3616/22873 22917/10824/22964 22918/10825/22965 22827/10756/2287

                            for (int i=1;i<tempsa.length-2;i++){
                                readFace(nowObj,v,vt,vn,tempsa[1]);
                                readFace(nowObj,v,vt,vn,tempsa[i+1]);
                                readFace(nowObj,v,vt,vn,tempsa[i+2]);
//                                readFace1(nowObj,v,vt,vn,tempsa,i);
                            }
                            break;
                    }
                }
    ...
    public static void readFace1(Obj3D nowObj,ArrayList<Float> v,ArrayList<Float> vt,ArrayList<Float> vn,String[] tempsa,int i){
        String[] fs=tempsa[i].split("/");
        int index;
        if(fs.length>0){
            //顶点索引
            index=Integer.parseInt(fs[0])-1;
            nowObj.addVert(v.get(index*3));
            nowObj.addVert(v.get(index*3+1));
            nowObj.addVert(v.get(index*3+2));
        }
        if(fs.length>1){
            //贴图
            index=Integer.parseInt(fs[1])-1;
            nowObj.addVertTexture(vt.get(index*2));
            nowObj.addVertTexture(vt.get(index*2+1));
        }
        if(fs.length>2){
            //法线索引
            index=Integer.parseInt(fs[2])-1;
            nowObj.addVertNorl(vn.get(index*3));
            nowObj.addVertNorl(vn.get(index*3+1));
            nowObj.addVertNorl(vn.get(index*3+2));
        }
    }
    public static void readFace(Obj3D nowObj,ArrayList<Float> v,ArrayList<Float> vt,ArrayList<Float> vn,String v1){
        // 22827/10756/2287
        String[] fs=v1.split("/");
        int index;
        if(fs.length>0){
            //格式 v
            index= Integer.parseInt(fs[0])-1;
            readV(nowObj,v,index);
        }

        if(fs.length>1){
            //格式   v/vt
            //纹理坐标可以省略 v//vn
            if(!TextUtils.isEmpty(fs[1])){
                index= Integer.parseInt(fs[1])-1;
                readVT(nowObj,vt,index);
            }
        }
        ...
        if(fs.length==3){
            //格式   v/vt/vn
            index= Integer.parseInt(fs[2])-1;
            readVN(nowObj,vn,index);
        }
    }

    public static void readV(Obj3D nowObj,ArrayList<Float> v,int index){
        nowObj.addVert(v.get(index*3));
        nowObj.addVert(v.get(index*3+1));
        nowObj.addVert(v.get(index*3+2));
    }
    public static void readVT(Obj3D nowObj,ArrayList<Float> vt,int index){
        nowObj.addVertTexture(vt.get(index*2));
        nowObj.addVertTexture(vt.get(index*2+1));
    }
    public static void readVN(Obj3D nowObj,ArrayList<Float> vn,int index){
        nowObj.addVertNorl(vn.get(index*3));
        nowObj.addVertNorl(vn.get(index*3+1));
        nowObj.addVertNorl(vn.get(index*3+2));
    }
